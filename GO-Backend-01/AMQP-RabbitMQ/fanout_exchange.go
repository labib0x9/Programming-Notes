...
func main() {
	...
	publish(ctx, conn, ex, route1, []byte("1 - only be in queue= all of three"))
	publish(ctx, conn, ex, route2, []byte("2 - only be in queue= all of three"))
	publish(ctx, conn, ex, route3, []byte("3 - only be in queue= all of three"))
